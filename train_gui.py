...
from utils.watermarking_utils import embedding_Watermark,decoding_Watermark, save_tensor_image
...
class GUI:
    def __init__(self, args, dataset, opt, pipe, testing_iterations, saving_iterations) -> None:
        ...
        self.watermarked = opt.watermarked
        self.watermark_message = opt.watermarked_message
    ...
    def train_step(self):
        ...
        # Watermarking (numpy_image ,tensor_image, watermarking message)
        print("**********************************************************")
        print("Watermarking : ", self.watermarked)
        if self.watermarked:
            _, watermarked_tensor_image ,watermarking = embedding_Watermark(viewpoint_cam.original_image, self.watermark_message)

            decoded_message = decoding_Watermark(watermarked_tensor_image)        
            viewpoint_cam.original_image = watermarked_tensor_image 

        
        # print(watermarking, decoded_message, '\n')
        """
        1. To train model from Watermarked Image, convert viewpoint_cam.original_image(no-watermarking) to Watermarked_image(watermarking)
        ...
        print("DATA NAME READING!!!!!! :", lp._source_path)
        print("******************************************************")
        save_tensor_image(viewpoint_cam.original_image, image, self.start_idx, self.data_name, self.watermarked) 
        self.start_idx +=1 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-name", type=str, default='lego')
