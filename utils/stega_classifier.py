...
class VGG16UNet(nn.Module):
    def __init__(self, in_channel=4, out_channel=3, num_filters=32, message_length=16, pretrained=True, requires_grad=True,args=None):
        """
        ...
        self.fc = nn.Sequential(
            nn.Linear(4 * 128 * 128, 4096),
            nn.ReLU(inplace=True),
            nn.Linear(4096, 4096),
            nn.ReLU(inplace=True),
            nn.Linear(4096, self.message_length)
        )
    ...
    def forward(self, x, c, return_c=False, cmask=False):
        ...
        c = F.interpolate(c,size=(128, 128))
